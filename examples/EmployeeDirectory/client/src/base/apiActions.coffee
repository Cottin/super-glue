{flatten, identity, remove, update, wrap} = require 'ramda' #auto_require:ramda
...
actionDeclarations =
	auth:
		login: a set, fail(loginError), post, ({email, password}) -> ['auth/login', {email, password}]
		logout: a resetData, unset, fail(logoutError), get, -> 'auth/logout'
		mock: a post, (user) -> ['auth/mock', user]
	employees:
		get: a set, get, -> 'employee'
		# actions for a single employee:
		update: a setById, put, (employee) -> ["employee/#{employee.id}", employee]
	employeeToEdit:
		editById: a set, getPathById('employees'), identity
	issues:
		get: a set, get, (searchQuery) -> ['issues', searchQuery]
